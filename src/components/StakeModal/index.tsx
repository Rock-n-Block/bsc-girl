...
const StakeModal: React.FC = observer(() => {
  ...
  useEffect(() => {
    if (user.address) {
      ...
      if (modals.stakeModal.operation !== 'Remove part of stake') {
        ...
        value = new BigNumber(modals.stakeModal.stakedAmount)
          .dividedBy(new BigNumber(10).pow(decimals))
          .toFixed(decimals, 1);
        setBalance(value);
        setStaked(value * 0.5);
  ...
  useEffect(() => {
    if (+staked < 0.00001) {
      setErrorMsg('Value can not be lower then 0.00001');
    } else setErrorMsg('');
  }, [staked]);
  ...
  return (
    ...
    >
      ...
      <div className="stake-modal__form">
        ...
        <div className={`stake-modal__form__input ${errorMsg ? 'red-border' : ''}`}>
          <InputNumber
            max={+balance}
            value={staked}
            positiveOnly
        ...
        <div className="stake-modal__form__error">{errorMsg}</div>
        ...
        {modals.stakeModal.operation !== 'Remove part of stake' ? (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="gradient-button get-currency"
          >
